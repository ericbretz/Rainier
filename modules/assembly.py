...
def assembly(r_csv, fa_file, snapf, outdir):
    ...
    def sc_metrics():
        for index, row in contig_df.iterrows():
            ...
            contig_df.loc[index, 'sCcov'] = round((row['length'] - row['basesuncovered']) / row['length'], 6)
            contig_df.loc[index, 'sCord'] = row['good']
            contig_df.loc[index, 'sCseg'] = row['basesuncovered']
            contig_df.loc[index, 'coverage'] = round(row['eff_count'] * row['length'] / row['eff_length'], 6)
        ...
        for line in file:
            ...
            if line.startswith('>'):
                ...
                if not first:
                    ...
                    if read_length > 1000:
                        n_over_1k += 1
                    if read_length > 10000:
                        n_over_10k += 1
                    orf = longest_orf(frame)
                    n_with_orf += 1 if orf > 149 else 0
                    orf_len_sum += orf 
                read_length = 0
                frame = ''
                first = False
    ...
    mean_orf_percent = round((300 * orf_len_sum) / (seqs * mean), 6)
    ...
    p_fragments_mapped   = round(fragments_mapped / fragments, 6)
    good_mappings        = tc['good'].sum()
    p_good_mappings      = round(good_mappings / fragments, 6)
    bad_mappings         = round(fragments_mapped - good_mappings, 6)
    potential_bridges    = (tc['bridges'] > 0).sum()
    bases_uncovered      = tc['basesuncovered'].sum()
    p_bases_uncovered    = round(bases_uncovered / bases, 6)
    p_contigs_uncovered  = round(contigs_uncovered / tc.shape[0], 6)
    p_contigs_lowcovered = round(contigs_lowcovered / tc.shape[0], 6)
    contigs_segmented    = round((tc['p_notsegmented'] < 0.5).sum(), 6)
    p_contigs_segmented  = round(contigs_segmented / tc.shape[0], 6)
    contig_uncovbase     = round((tc['basesuncovered'] > 0).sum(), 6)
    p_contig_uncovbase   = round(contig_uncovbase / tc.shape[0], 6)
    ...
    def optimal_score():
        opt_product   = 0
        opt_good      = 0
        opt_product   = tc['scores'].apply(math.log).sum()
        opt_good      = tc['good'].sum()
        opt_count     = len(tc)
        cutoffscores  = {}
        scores_sorted = tc[['scores', 'good']].sort_values(by='scores')
        ...
        cuttofffile     = os.path.join(outdir, 'rainier', 'assembly_score_optimisation.csv')
    ...
    outfile    = os.path.join(outdir, 'rainier', 'assembly.csv')
    output_df.to_csv(outfile, index=False)

    contigfile = os.path.join(outdir, 'rainier', 'contigs.csv')
